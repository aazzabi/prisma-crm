...
public class ClaimService implements IClaimServiceLocal, IClaimServiceRemote {
	...
	public Claim getByCode(String code) {
		Claim c = (Claim) em.find(Claim.class, code);
		return c;
	}
	...
	@Override
	public void changeStatus(Claim c, ClaimStatus status) {c.setStatus(status);}
	@Override
	public void changePriority(Claim c, ClaimPriority priority) {c.setPriority(priority);}
	@Override
	public void deleteClaim(Claim c) {em.remove(c);}
	@Override
	public void deleteClaimById(int id) {em.remove(em.find(Claim.class, id));}
	@Override
	public void deleteClaimByClient(Client c) {em.remove(em.find(Claim.class, c));		}
	...
	public Agent findAnAgentFreeAndActif() {
		Agent a;
		TypedQuery<Agent> queryDispo=em.createQuery("SELECT a from Agent a where a.dispoClaim =:d ORDER BY a.nbrClaims", Agent.class).setParameter("d", "disponible");
		if (queryDispo.getSingleResult()!= null) {
			...
			TypedQuery<Agent> query = em.createQuery("SELECT a from Agent a ORDER BY DESC a.nbrClaims", Agent.class).setParameter("d", "disponible");
...
// -----------------------------------------------------------------------------
	...
	public String getAllClaimJSON(List<Claim> listC) {
		...
		ObjectNode main = mapper.createObjectNode();
		try {
			 test=mapper.writeValueAsString(listC);
		} catch (JsonProcessingException e) {
			e.printStackTrace();
		}
		return test;
