...
public class InvoiceService implements IInvoiceLocal {
	...
	public String getCurrencyCurrentValue(String base, String trgt, int invoiceId) {
		...
		if (invoice != null) {
			...
			WebTarget target = client.target(CURRENCY_API_URL).path(Float.toString(invoice.getOrder().getTotale()))
					.path(base).path(trgt);
	...
	public List<Invoice> fetchInvoices() {
		...
		return manager.createQuery("SELECT I FROM Invoice I").getResultList();
	}
	...
	public String getInvoiceProductRows(int invoice) {
		...
		String result = "";
		if (INVOICE != null) {
			ClientOrder order = (ClientOrder) manager
					.createQuery("SELECT O FROM ClientOrder O WHERE O.invoice=:invoice")
					.setParameter("invoice", INVOICE).getSingleResult();
			if ((order != null) && (order.getCart().getCartRows().size() > 0)) {
				for (CartProductRow row : order.getCart().getCartRows()) {
					...
					if (row.getUsedFidelityPoints() == 0) {
						result += "\nReduction is being applied in this row with the use of "
								+ row.getUsedFidelityPoints() + "\n\n";
					} else
